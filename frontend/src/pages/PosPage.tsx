...
export function PosPage() {
  ...
  return <>
    <PageHeader eyebrow="Cashier workspace" title="Point of Sale" description="Build an order from live inventory and complete a secure cash payment." actions={<button className="button scan-button" onClick={() => setScanner(true)}>▣ Scan Barcode</button>} />
    {message && <Notice tone="error">{message}{message === 'Product not registered in inventory' && <> · <a href="/">Register product</a></>}</Notice>}
    <div className="pos-layout" aria-label="Point of sale workspace">
      <section className="card product-picker"><div className="pos-panel-heading"><div><span className="eyebrow">Live catalog</span><h2>Find products</h2></div><span>{results.length} available results</span></div><form className="pos-search" onSubmit={(event: FormEvent) => { event.preventDefault(); void search() }}><label className="sr-only" htmlFor="pos-search">Search name, SKU, or barcode</label><input id="pos-search" autoFocus placeholder="Search product name, SKU, or barcode" value={query} onChange={(event) => setQuery(event.target.value)} /><button className="button primary">Search catalog</button></form>
        <div className="pos-results" aria-live="polite">{results.length === 0 ? <EmptyState title="No matching products" description="Try a different name, SKU, or barcode." action={<button className="button secondary" onClick={() => { setQuery(''); void search('') }}>Show all products</button>} /> : results.map((product) => <article key={product.id} className="result-item"><div className="result-main"><span className={`stock-dot ${product.current_stock ? 'available' : ''}`} aria-hidden="true"/><div><strong>{product.name}</strong><span><code>{product.sku}</code>{product.barcode ? ` · ${product.barcode}` : ''}</span><small>{product.current_stock} in stock</small></div></div><div className="result-purchase"><strong>{money(cents(product.selling_price))}</strong><button className="button secondary" disabled={product.current_stock === 0} onClick={() => add(product)}>{product.current_stock ? 'Add' : 'Out of stock'}</button></div></article>)}</div>
      </section>
      <aside className="card cart-panel" aria-label="Current order"><div className="cart-heading"><div><span className="eyebrow">Current order</span><h2>Order summary</h2><small>{cart.reduce((n, line) => n + line.quantity, 0)} items in cart</small></div><button className="text-button danger" disabled={!cart.length} onClick={() => setCart([])}>Clear</button></div>
        {!cart.length ? <EmptyState title="Your order is empty" description="Search or scan a product, then add it to this order." /> : <div className="cart-lines">{cart.map((line) => <article key={line.product.id} className="cart-line"><div className="cart-product"><strong>{line.product.name}</strong><span>{line.product.sku} · {money(cents(line.product.selling_price))} each</span><small>{line.product.current_stock} in stock</small></div><div className="quantity"><button aria-label={`Decrease ${line.product.name}`} onClick={() => adjust(line.product.id, line.quantity - 1)}>−</button><input aria-label={`${line.product.name} quantity`} type="number" min="1" max={line.product.current_stock} value={line.quantity} onChange={(event) => adjust(line.product.id, Number(event.target.value))} /><button aria-label={`Increase ${line.product.name}`} disabled={line.quantity >= line.product.current_stock} onClick={() => adjust(line.product.id, line.quantity + 1)}>+</button></div><strong>{money(cents(line.product.selling_price) * line.quantity)}</strong><button className="text-button danger" aria-label={`Remove ${line.product.name}`} onClick={() => setCart((current) => current.filter((item) => item.product.id !== line.product.id))}>Remove</button></article>)}</div>}
        <div className="payment"><dl><div><dt>Subtotal</dt><dd>{money(total)}</dd></div><div className="grand-total"><dt>Order total</dt><dd>{money(total)}</dd></div></dl><div className="payment-field"><label htmlFor="cash">Cash received</label><small>Enter the amount tendered by the customer.</small><div className="money-input"><span>₱</span><input id="cash" type="number" inputMode="decimal" min="0" step="0.01" value={cash} onChange={(event) => setCash(event.target.value)} placeholder="0.00" /></div></div><div className="change-display"><span>Change due</span><strong>{money(Math.max(0, tendered - total))}</strong></div><button className="button primary checkout" aria-label="Checkout" disabled={!canCheckout} onClick={() => void checkout()}>{loading ? 'Processing payment…' : `Checkout · ${money(total)}`}</button>{cart.length > 0 && tendered < total && <small className="payment-hint">Enter at least {money(total)} to complete checkout.</small>}</div>
      </aside>
    </div>
