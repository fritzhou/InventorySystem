...
export function AuthProvider({children}:{children:ReactNode}) {
  ...
  const login=async(email:string,password:string)=>{const authenticated=await api.login(email,password);setUser(authenticated);navigate(authenticated.must_change_password?'/account':authenticated.role==='ADMIN'?'/dashboard':'/pos')}
