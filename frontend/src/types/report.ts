export interface ReportSummary { sales_total: string; transaction_count: number; items_sold: number; gross_profit: string; profit_complete: boolean; average_transaction_value: string; total_active_products: number; total_units_in_stock: number; low_stock_count: number; out_of_stock_count: number }
export interface TrendPoint { date: string; sales: string; transactions: number; items_sold: number }
