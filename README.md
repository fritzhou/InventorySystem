...
The Products screen now provides catalog creation, editing, search, category filtering, category creation, stock-level visibility, and product deactivation. Product forms collect the identifiers and commercial fields needed by retail operations: SKU, optional barcode, category, decimal cost/selling prices, opening stock, and the minimum-stock threshold. Stock is shown as **In stock**, **Low stock**, or **Out of stock**; a positive quantity is low when it is less than or equal to its configured threshold.

Creating or editing a product sends JSON through the centralized frontend API service to `POST /api/products` or `PATCH /api/products/{id}`. FastAPI validates the Pydantic schema, verifies the category foreign key, and lets PostgreSQL constraints protect unique SKUs/barcodes and nonnegative values. API validation and conflict details are shown in the form rather than replaced by a generic failure message.

The edit form intentionally does not change `current_stock`. Opening stock can be recorded while creating a product, but later stock changes will go through the Inventory module so every adjustment can produce a traceable stock-movement record. Deactivation uses `DELETE /api/products/{id}` as the REST action but performs a soft delete (`is_active = false`); the database row remains intact for future sales and audit history.

These Phase 2 routes remain unauthenticated until the planned authentication milestone. They are suitable for local development but must not be publicly exposed before admin/cashier authorization is enforced by FastAPI.
